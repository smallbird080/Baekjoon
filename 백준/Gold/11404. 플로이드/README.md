...
문제에서 한 도시 쌍 (A,B) 사이 여러 도로가 존재 가능하다고 하였으므로, A-B weight를 최솟값으로 설정해야 함.

INF 값을 잡을 때 넉넉하게, 문제에서 나올 수 있는 최악의 값 + 1 정도로 잡기. 이 문제에서는 A->B 에서 모든 도시를 거쳐가는 경우 최대 도시&W 100,000*(100-1)에 +1 값으로 설정
