...
D가 나오면 Array 시작 메모리 주소를 바꾸고 Array 길이를 줄여 접근 가능한 메모리 범위의 제한을 두었다.

R이 나오면 메모리에 접근하는 순서를 전환하도록 하였다.

이를 통해 R,D 연산 하나를 O(1)안에 마무리하도록 하였고, 연산의 결과는 print시에 array를 읽는 방향과 범위를 조정하는 식으로 나타나게 하였다.
