...
최소 경로를 구하기 위해 벨만 포드를 사용하는 것이 아닌, **negative cycle의 존재 여부 판단**을 목적으로 함.

따라서 n-1번의 RELAX 이후 모든 edge에 대해 추가 update 여부 (negative cycle 존재) 확인하여 모든 v에 대해 init. vertex 지정 검사를 하지 않고도 cycle 판별.
